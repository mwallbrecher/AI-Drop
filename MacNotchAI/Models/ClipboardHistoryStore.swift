...
final class ClipboardHistoryStore: ObservableObject {
    ...
    // Sensitive / transient pasteboard types we must never record.
    ...
    private func capture(from pb: NSPasteboard) {
        // Privacy: never record sensitive / transient items.
        let present = Set((pb.types ?? []).map(\.rawValue))
        guard present.isDisjoint(with: excludedTypes) else { return }
