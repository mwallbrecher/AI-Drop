...
returns the new `URL`. `UI/FileToolsMenu.swift` (`FileToolActions`) dispatches them: file-PRODUCING ops
...
← back (`vm.returnToChips()`) actions. Facts come from **`Core/FileFacts.swift`** — a `nonisolated`,
`Sendable` `FileFacts.gather(_:) async` that probes off-main (CGImageSource dims, PDFKit pages,
`AVURLAsset.load(.duration)`, recursive folder size/count) and is awaited from a `.task`. Both
`remapSessionURL` switches carry the `.fileResult` case so a rename remaps both URLs.
